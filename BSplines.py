...
class BSplines:
    ...
    def __init__(self, knot_vector, order, grid_size=1000):
    ...
    def get_b_spline_functions(self):
        try:
        ...
        x_max = np.max(self.knot_vector) - 0.0000000000001

        x = np.linspace(x_min, x_max, num=self.grid_size)

        return np.array([self(i) for i in x]).T
